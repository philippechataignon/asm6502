...
;   PRINT A$(12,14)
;
PAREN_SUBSTR    jsr     LE118
                sta     NOUNSTKL,x
                cmp     NOUNSTKH,x
LE102           bcc     LE115
PrintErr05      ldy     #<ErrMsg05        ;"STRING"
LE106           jmp     ERRMESS
...
PrintErr03      ldy     #<ErrMsg03        ;"MEM FULL"
                bne     LE106             ;(always)
...
;   X = 27 - 2
;
SUBTRACT        jsr     NEGATE            ;negate, then add
;
; Token $12 +
;   num op
;   X = 27 + 2
...
unref_branch   bne     LE7AE             ;no, adjust CH [always?]
...
STOPPED_AT      ldy     #<ErrMsg12        ;"STOPPED AT"
                jsr     ERRORMESS
                ldy     #$01
                lda     (PR),y
                tax
...
NEXT            ldy     #<ErrMsg11        ;"BAD NEXT"
                lda     FORNDX
JmpERRMESS2     beq     JmpERRMESS1       ;no more FORs
                tay
                lda     NOUNSTKL,x
                cmp     STK_40-1,y
                bne     LE8D6
...
HI255ERR        ldy     #<ErrMsg15        ;">255"
JmpERRMESS3     jmp     ERRMESS

RANGERR         ldy     #<ErrMsg16        ;"RANGE"
                bne     JmpERRMESS3
...
unref_ef80      jsr     GET16BIT
                ...
                lda     ACC+1
                sta     HIMEM+1
unref_branch2   jmp     NEW

unref_ef96      jsr     GET16BIT
                ldy     ACC               ;compare ACC and LOMEM
                cpy     HIMEM
                lda     ACC+1
                sbc     HIMEM+1
...
unref_f028      jsr     from_unref
                jmp     LE8BE

LF02E           ldx     PX
                lda     PX+1
from_unref     ldy     KBD               ;get keypress
...
; Token $7a NOTRACE
;
NOTRACE         lsr     NOUNSTKC          ;clear bit 7
                rts
...
;   string var
;
DSP_STR         inx                       ;[DSP_NUM in paulrsm disasm]
;
; Token $7c DSP
;   num var
;
